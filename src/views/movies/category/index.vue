...
import { ref } from "vue";
import { useMoviesCategory } from "./utils/hook";
...
} = useMoviesCategory(tableRef);
</script>

<template>
  <div v-if="hasAuth('list:MoviesCategory')" class="main">
    <el-form
      ref="formRef"
      :inline="true"
      :model="form"
    ...
    >
      <el-form-item :label="t('MoviesCategory.name')" prop="name">
        <el-input
          v-model="form.name"
    ...
    <PureTableBar
      :title="t('MoviesCategory.category')"
      :columns="columns"
      ...
      <template #buttons>
        <el-space wrap>
          <div v-if="manySelectCount > 0" class="w-[360px]">
            ...
            <el-popconfirm
              v-if="hasAuth('manyDelete:MoviesCategory')"
              :title="
          ...
          <el-button
            v-if="hasAuth('create:MoviesCategory')"
            type="primary"
      ...
      <template v-slot="{ size, dynamicColumns }">
        ...
        >
          ...
          <template #operation="{ row }">
            <el-button
              v-if="hasAuth('update:MoviesCategory')"
              class="reset-margin"
              link
              type="primary"
              :size="size"
            ...
            <el-popconfirm
              v-if="hasAuth('delete:MoviesCategory')"
