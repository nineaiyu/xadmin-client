/**
 * 角色权限树合成键约定（`utils/hook.tsx` 构树 ↔ `utils/permissionTree.ts` 解析
 * 的单一事实源）。
 *
 ...
 *
 * 勾选结果按此约定反解（`collectSelectionPayload`）：无 "+" → 菜单授权；
 * `a+b` → 菜单 a 勾选字段 b（按菜单分组合并进 role.fields）；`+x` 不计入授权。
 * 合成节点不参与菜单父子联动，也不计入菜单状态统计。
 * 修改键格式属破坏性变更：需同步 `MenuPermissionTree` 的勾选回显
 * （setCheckedKeys）与后端 role.fields 的读取口径，并回归角色授权 E2E。
 */
