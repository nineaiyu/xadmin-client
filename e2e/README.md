...
| `pnpm <script> -- <args>` 透传时 `--` 是 **pnpm 的参数分隔符**，写 `-- --project=x` 在视觉上像「两个 `--`」，容易被误读为多写/漏传（2026-09-15 实测讨论）                                                                                                       | CI 与脚本里**直调** `pnpm exec playwright test <file> --project=x`（零歧义）；本地仍沿用本页「运行」表的 `pnpm test:e2e -- e2e/xxx.e2e.ts` 示例                                                                                                                                                                                                                                  |
